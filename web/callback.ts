...
serve(async (req)=>{
  try {
    ...
    for (const t of tracks){
      ...
      // public URL ---------------------------------------------------------
      const publicUrl = t.audio_url || t.source_audio_url;
      
      // Update existing DB record with completed song data ----------------
      console.log(`🔄 Updating song record with task_id: ${taskId}, track_id: ${id}`);
      ...
      if (updateError) {
        console.error("❌ Database update failed for task_id:", taskId, updateError);
        results.push({
          id,
          success: false,
          error: `database_update_failed: ${updateError.message}`
        });
        continue;
      }

      console.log("✅ Updated song record for task_id:", taskId, "track_id:", id);
      results.push({
        id,
        success: true,
        publicUrl,
